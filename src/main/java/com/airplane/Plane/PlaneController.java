...
public class PlaneController {
	...
	public ResponseEntity createPlane(@PathVariable("flightNumber") String flightnumber,
		...
		try {
			Plane updFlight = planeService.getFlight(flightnumber); 
			if( updFlight != null ) {
				...
				if(updFlight.getPlane().getCapacity() - updFlight.getSeatsLeft() > capacity) {
					...
					if( checkOverlap(passengers, flightnumber, depT, arrT)) {
						...
						planeinfo.setCapacity(capacity);
						planeinfo.setManufacturer(manufacturer);
						planeinfo.setModel(model);
						planeinfo.setYearOfManufacture(yearOfManufacture);
						...
						JSONObject responseFlight = new JSONObject(buildFlightResponse(planeService.createFlight(updFlight)));
				...
				JSONObject responseFlight = new JSONObject(buildFlightResponse(planeService.createFlight(flight)));
				return new ResponseEntity(XML.toString(responseFlight), getXMLHeaders(), HttpStatus.OK);
			}
		} catch(Exception ex) {
	...
	public ResponseEntity getFlight(@PathVariable("number") String number, 
		...
		try {
			if(planeService.getFlight(number) != null) {
				JSONObject flight = new JSONObject(buildFlightResponse(planeService.getFlight(number)));
				if(xml != null) {
	...
	public ResponseEntity deleteFlight(@PathVariable("number") String number) {
		try {
			Plane flight = planeService.getFlight(number);
			if(flight != null) {
				List<Passenger> passengers = flight.getPassengers();
				if(passengers == null || passengers.size() == 0) {
					planeService.deleteFlight(flight);
	...
	public Map<String, Object> buildFlightResponse(Plane flight) throws ParseException {
	...
	public boolean checkOverlap(List<Passenger> passengers, String flightnumber, Date depT, Date arrT) {
		...
		for(Passenger p : passengers) {
			...
			for(Reservation r : reservations) {
				List<Plane> flights = r.getFlights();
				for(Plane f : flights) {
